...
# Common API keys we may want to acknowledge without leaking the
_SENSITIVE_KEYS = [
    "OPENAI_API_KEY",
    "OPENAI_ORG_ID",
...
# Treat any env var starting with these prefixes as sensitive
_SENSITIVE_PREFIXES = [
    "DOCKER_AUTH_",
]
...
def collect_env() -> Dict[str, str]:
    ...
    for k in _SENSITIVE_KEYS:
        ...
        if v:
            out[k] = _redact(v)
    # also scan for prefixed keys
    for k, v in os.environ.items():
        if any(k.startswith(pref) for pref in _SENSITIVE_PREFIXES):
            out[k] = _redact(v)
    return out

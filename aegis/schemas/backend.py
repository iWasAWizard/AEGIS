...
class BaseBackendConfig(BaseModel):
    ...
    type: str = Field(..., description="The type of backend provider (e.g., 'koboldcpp', 'openai').")
...
class KoboldcppBackendConfig(BaseBackendConfig):
    ...
    llm_url: str = Field(..., description="The full URL to the KoboldCPP /generate endpoint.")
...
class OllamaBackendConfig(BaseBackendConfig):
    ...
    llm_url: str = Field(..., description="The full URL to the Ollama /api/generate endpoint.")
...
class OpenAIBackendConfig(BaseModel):
    ...
    model: str = Field(default="gpt-4-turbo", description="The model name to use for completions.")
    api_key: str = Field(..., description="The API key for the OpenAI service.")
    temperature: float = Field(0.7)
    max_tokens_to_generate: int = Field(2048)
    top_p: float = Field(1.0)
    tts_model: str = Field(default="tts-1", description="The model name for text-to-speech.")
    tts_voice: Literal["alloy", "echo", "fable", "onyx", "nova", "shimmer"] = Field(
        default="alloy", description="The voice to use for text-to-speech."
    )
    stt_model: str = Field(default="whisper-1", description="The model name for speech-to-text.")
